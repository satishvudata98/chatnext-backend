...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/api/conversations") {
        ...
        if (searchError)
            return sendJSON(res, 500, { success: false, message: searchError.message });
        ...
        if (conversationId) {
            return sendJSON(res, 200, { success: true, conversation: { id: conversationId } });
        }
        ...
        if (insertConvError)
            return sendJSON(res, 500, { success: false, message: insertConvError.message });

        // Add both users to conversation_members
        const { error: insertMembersError } = await supabase
            .from("conversation_members")
            .insert([
                { conversation_id: conversationId, user_id: decoded.userId, joined_at: now },
                { conversation_id: conversationId, user_id: otherUserId, joined_at: now }
            ]);

        if (insertMembersError)
            return sendJSON(res, 500, { success: false, message: insertMembersError.message });
